...
def check_delimiters(expr):
    ...
    test = Stack()
    for m in expr:
      if m in delim_openers or m in delim_closers:
        test.push(m)
        total+=1
    if total%2!=0:
      answer = False
    ...
    for i in expr:
      ...
      if i in delim_closers: 
        if delim_openers[delim_closers.find(i)]!=stack.pop():
          answer = False
        else:
          countPop = countPop+1
    if countPush==countPop and stack.empty():
      answer = True
    return answer
    ### END SOLUTION
...
def infix_to_postfix(expr):
    ...
    while i<len(toks):
      ...
      elif ops.empty() or ops.peek()=="(" or toks[i]=="(":
        ops.push(toks[i])
      elif toks[i]==")":
        while ops.peek() != "(":
          postfix.append(ops.pop())
        ops.pop()
      elif prec[toks[i]] > prec[ops.peek()]:
        ops.push(toks[i])
      elif prec[toks[i]] == prec[ops.peek()]:
        postfix.append(ops.pop())
        ops.push(toks[i])
