...
class HBStree:
    ...
    def insert(self, key):
        ...
        def recadd(node):
            ...
            if node.val > key:
                if node.left:
                    return self.INode(node.val, recadd(node.left), node.right)
                return self.INode(node.val, self.INode(key, None, None),
                                      node.right)
            if node.val < key:
                if node.right:
                    return self.INode(node.val, node.left, recadd(node.right))
                return self.INode(node.val, node.left,
                                      self.INode(key, None, None))
        ...
        if self.__contains__(key)==False:
            ...
            if node is not None:
                self.root_versions.append(recadd(node))
        # END SOLUTION
    ...
    def delete(self, key):
        ...
        def rec_delete(node):
            if node.val < key:
                return self.INode(node.val, node.left, rec_delete(node.right))
            if node.val > key:
                return self.INode(node.val, rec_delete(node.left), node.right)
            if not node.right:
                return node.left
            if not node.left:
                return node.right
        ...
        if self.__contains__(key):
            node = self.root_versions[-1]
            self.root_versions.append(rec_delete(node))
        # END SOLUTION
    ...
    def subtree_size(node):
        ...
        if not node:
            ...
            return 1 + HBStree.subtree_size(node.left) + HBStree.subtree_size(
                node.right)
...
def main():
    for t in [
            test_insert_1, test_insert_2, test_lookup, test_delete_1,
            test_delete_2, test_corner_cases
    ]:
        say_test(t)
        t()
        say_success()
