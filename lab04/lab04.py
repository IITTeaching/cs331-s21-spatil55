...
class ArrayList:
    ...
    def index(self, value, i=0, j=None):
        ...
        if j and self._normalize_idx(j) < self.len:
          for x in range(i,  self._normalize_idx(j)):
            if self.data[x] == value:
              return x
        else:
          for a in range(i, self.len):
            if self.data[a]==value:
              return a
    ...
    def __add__(self, other):
        ...
        of other."""
        ### BEGIN SOLUTION
        copy = ArrayList(self.len + other.len)
        for i in range(self.len):
          copy[i] = self.data[i]
        for j in range(other.len):
          copy[self.len+j] = other[j]
        return copy
        ### END SOLUTION
    ...
    def extend(self, other):
        ...
        ### BEGIN SOLUTION
        finish = ConstrainedList(self.len + len(other))
        for i in range(self.len):
          finish[i] = self.data[i]
        for j in range(len(list(other))):
          finish[self.len+j] = list(other)[j]
